...
module Jekyll
  ...
  class ApiResourcePage < Page
    ...
    def initialize(site, base, dir, key_prefix, singular_rsrc_name, raml, raml_resource, raml_method, subpages)
      ...
      # generate snippet examples
      # for curl and javascript, replace "Accept" with "Content-Type" and outer double quotes with single quotes
      # for ruby, just replace "Accept" with "Content-Type"
      self.data['raml_snippets'] = [
        {
          'title' => 'CURL',
          'snippet' => begin
            snippet_generator.curl(raml_resource, raml_method.method.downcase)
              .sub('-H "Accept:', '-H "Content-Type:')
              .sub(/[\"](?=[{])/, '\'')
              .sub(/[\"](?=.*\z)/, '\'')
          rescue
            ''
